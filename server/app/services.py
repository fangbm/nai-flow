...
async def generate_storyboard(payload: ScriptRequest) -> dict | None:
    ...
    system = (
        ...
        'Also add one prompt-ready English layout declaration that the caller will place as the first sentence of the NovelAI prompt. It must begin with the panel count and `manga page` or `comic`, explicitly state every panel\'s position and relative size, and include `white panel borders`. For an irregular page, directly name the structural relationship (for example `one large main illustration with a narrow top inset and a small lower-right reaction inset`); never write meta language such as `adaptive layout`, `choose a layout`, or `the layout is`. '
        'Every `panels` entry must be a self-contained numbered or positional line that begins with its panel anchor, so entries can be kept on separate lines in the final NovelAI prompt. '
